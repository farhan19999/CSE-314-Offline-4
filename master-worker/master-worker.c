...
//Thread locks and variables
pthread_mutex_t mlock = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t mcond = PTHREAD_MUTEX_INITIALIZER;
...
void *generate_requests_loop(void *data) {
    ...
    while(1) {
        ...
        pthread_mutex_lock(&mcond);

        while(curr_buf_size == max_buf_size) {
            pthread_cond_wait(&consumed, &mcond);
        }
...
void *consume_requests_loop(void *data) {
    ...
    while(1) {
        ...
        while(item_to_consume <total_items && curr_buf_size == 0) {
            pthread_cond_wait(&produced, &mcond);
        }
        ...
        int t = buffer[--curr_buf_size];
        print_consumed(t, thread_id);
        item_to_consume++;
        if(curr_buf_size == 0){ 
            pthread_cond_signal(&consumed);
        }
        else pthread_cond_signal(&produced);
        pthread_mutex_unlock(&mcond);
    }
    return 0;
}
